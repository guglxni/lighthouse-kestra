...
const shimmerSurfaceClass =
  "group relative z-0 inline-flex cursor-pointer items-center justify-center overflow-hidden whitespace-nowrap border border-white/10 px-6 py-3 font-semibold text-ink-50 [background:var(--bg)] [border-radius:var(--radius)] transform-gpu transition-transform duration-300 ease-out active:translate-y-px hover:-translate-y-0.5";
...
export const ShimmerButton = React.forwardRef<HTMLButtonElement, ShimmerButtonProps>(
  (
    ...
    if (href) {
      return (
        <Link href={href} className={cn(shimmerSurfaceClass, className)} style={style}>
    ...
    return (
      <button ref={ref} type={type} style={style} className={cn(shimmerSurfaceClass, className)} {...props}>
        <ShimmerChrome>{children}</ShimmerChrome>
      </button>
    );
  },
